...
function affine_invariant_mcmc_firstrun(n_ensemble, n_walkers, n_iterations, target_probability, test_case)
    ...
    for i in 1:n_ensemble
        ...
        "k_5", "k_6", "k_7", "G"], progress=false, initial_params=previous_chains[i])
    end
...
function affine_invariant_mcmc(n_ensemble, n_walkers, n_iterations, target_probability, sub_chain, test_case)
    ...
    for i in 1:n_ensemble
        ...
        previous_chains[i] = deserialize(string("outputs/posterior_samples_$(test_case)_ensemble$(i)_walkers$(n_walkers)_iter$(n_iterations)_$(sub_chain-1).jls")).value
